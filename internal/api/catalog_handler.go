...
//
// No curated fallback: the pre-migration #19 branch carried a static
// slice of demo endpoints as a fallback when no SDKs published routes.
// That coupled the catalog to a demo-services list; the new design
// returns `{"data": [], ...}` and a `hint` so the UI can show an
// "SDKs not publishing routes" empty state.
// =========================================================================
...
//
// Cross-service dependencies in `depends_on` are passed through; intra-
// service references are normalized to "METHOD /path" form. Unresolvable
// references (target route absent from the assembled catalog) are dropped
// at the consumer; the catalog itself does not validate.
func endpointsFromInstances(instances []*model.SDKInstance) []CatalogEndpoint {
	...
	// Stable order — the picker shows entries grouped by service then path.
	sortEndpoints(out)
	return out
}
...
// sortEndpoints sorts by service, then method, then path so the UI gets
// a deterministic list.
func sortEndpoints(eps []CatalogEndpoint) {
	// Manual insertion sort — list is tiny (one entry per route across
	// the cluster) and avoids pulling in sort.Slice for one call.
	for i := 1; i < len(eps); i++ {
		j := i
		for j > 0 && lessEndpoint(eps[j], eps[j-1]) {
