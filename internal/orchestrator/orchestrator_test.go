...
type fakeZeus struct {
	...
	// runStats / statsMisses back GET /runs/{id}/stats (harvest_test.go):
	// canned finalized stats per run, and a count of 404s served.
	runStats    map[string]*fakeRunStats
	statsMisses map[string]int
...
func newFakeZeus(t *testing.T) *fakeZeus {
	...
	f := &fakeZeus{
		...
		runDatasets: make(map[string]string),
		runReasons:  make(map[string]string),
